...
foreach ($files as $f) {
    ...
    if ($start_pos !== false) {
        ...
        if ($navbar_pos !== false) {
            ...
            // Note: we extract styles to $extra_css
            $header_str = "<?php\n\$page_title = '$title';\nob_start();\n?>\n";
            // We will capture anything between <head> and </head> that is a <style> block
    ...
    if ($footer_start !== false) {
        ...
        // We capture any script tags that are NOT bootstrap or sidebarCollapse
        ...
        // Now wrap the remaining scripts in ob_start()
